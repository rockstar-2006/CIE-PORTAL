const { app, BrowserWindow, Menu, globalShortcut, ipcMain, session, screen } = require('electron');
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    alwaysOnTop: true,
    skipTaskbar: false,      // Setting this to false helps Windows recognize it as standard fullscreen over the taskbar
  ...
  // Makes the window content appear BLACK in ALL capture tools
  ...
  globalShortcut.register('PrintScreen', () => { notifyScreenshotBlocked(); return false; });
  globalShortcut.register('Alt+PrintScreen', () => { notifyScreenshotBlocked(); return false; });
  globalShortcut.register('Super+Shift+S', () => { notifyScreenshotBlocked(); return false; });
  globalShortcut.register('Super+PrintScreen', () => { notifyScreenshotBlocked(); return false; });
  globalShortcut.register('CommandOrControl+PrintScreen', () => { notifyScreenshotBlocked(); return false; });
  ...
  globalShortcut.register('Alt+Tab', () => { forceFocus(); return false; });
  globalShortcut.register('Alt+Shift+Tab', () => { forceFocus(); return false; });

  // Block Alt+F4 (Force close)
  globalShortcut.register('Alt+F4', () => { return false; });

  // Block Alt+Escape (Cycle windows)
  globalShortcut.register('Alt+Escape', () => { return false; });
...
app.on('ready', () => {
  ...
  session.defaultSession.setPermissionRequestHandler((webContents, permission, callback) => {
    ...
    const allowed = ['fullscreen', 'pointerLock', 'clipboard-read', 'clipboard-sanitized-write'];
